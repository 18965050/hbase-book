...
public class GetListExample {
	...
	public static void main(String[] args) throws IOException {
		...
		List<Get> gets = new ArrayList<Get>(); // co GetListExample-2-CreateList Create a list that holds the Get
												// instances.
		...
		for (Result result : results) {
			...
			byte[] val = null;
			if (result.containsColumn(cf1, qf1)) { // co GetListExample-5-GetValue1 Iterate over results and check what
		...
		for (Result result : results) {
			for (Cell cell : result.listCells()) { // co GetListExample-6-GetValue2 Iterate over results again, printing
													// out all values.
				System.out.println(
						"Row: " + Bytes.toString(cell.getRowArray(), cell.getRowOffset(), cell.getRowLength()) + // co
																													// GetListExample-7-GetValue2
																													// Two
																													// different
																													// ways
																													// to
