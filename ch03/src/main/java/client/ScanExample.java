...
public class ScanExample {
	...
	public static void main(String[] args) throws IOException {
		...
		ResultScanner scanner1 = table.getScanner(scan1); // co ScanExample-2-GetScanner Get a scanner to iterate over
															// the rows.
		...
		// vv ScanExample
		Scan scan2 = new Scan();
		scan2.addFamily(Bytes.toBytes("colfam1")); // co ScanExample-5-AddColFam Add one column family only, this will
													// suppress the retrieval of "colfam2".
		...
		scan3.addColumn(Bytes.toBytes("colfam1"), Bytes.toBytes("col-5"))
				.addColumn(Bytes.toBytes("colfam2"), Bytes.toBytes("col-33")). // co ScanExample-6-Build Use fluent
																				// pattern to add specific details to
																				// the Scan.
		...
		// vv ScanExample
		Scan scan4 = new Scan();
		scan4.addColumn(Bytes.toBytes("colfam1"), Bytes.toBytes("col-5")). // co ScanExample-7-Build Only select one
																			// column.
				setStartRow(Bytes.toBytes("row-10")).setStopRow(Bytes.toBytes("row-20"));
		...
		// vv ScanExample
		scan5.addColumn(Bytes.toBytes("colfam1"), Bytes.toBytes("col-5")).setStartRow(Bytes.toBytes("row-20"))
				.setStopRow(Bytes.toBytes("row-10")).setReversed(true); // co ScanExample-8-Build One column scan that
																		// runs in reverse.
