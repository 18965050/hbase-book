...
public class ClusterStatusExample {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		System.out.println("\nServer Info:\n--------------");
		for (ServerName server : status.getServers()) { // co ClusterStatusExample-2-ServerInfo Iterate over the
														// included server instances.
			...
			ServerLoad load = status.getLoad(server); // co ClusterStatusExample-3-ServerLoad Retrieve the load details
			...
			System.out.println("\nRegion Load:\n--------------");
			for (Map.Entry<byte[], RegionLoad> entry : // co ClusterStatusExample-4-Regions Iterate over the region
														// details of the current server.
			load.getRegionsLoad().entrySet()) {
				System.out.println("Region: " + Bytes.toStringBinary(entry.getKey()));

				RegionLoad regionLoad = entry.getValue(); // co ClusterStatusExample-5-RegionLoad Get the load details
